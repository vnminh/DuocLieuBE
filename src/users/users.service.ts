...
export class UsersService {
  ...
  private readonly defaultPassword = bycrpt.hashSync(
    '123456',
    this.saltOrRound,
  );
  ...
  ): Promise<ResponseCreateManyUserDto> {
  ...
  async verifyCode(data: {
    ...
    if (latest_code.code === data.verification_code) {
      ...
      return UsersMapper.toResponseVerifyCodeDto(
        user_with_reset_pass,
        data.purpose,
      );
    } else {
  ...
  async allUsers(filter: {
    ...
    return UsersMapper.toResponseAllUserDto(
      all,
      total,
